...
bool Renderer::check_device_extensions_support(const VkPhysicalDevice &device) {
    ...
    return m_required_extensions.size() == (size_t)required_found;
}
...
void Renderer::create_logical_device() {
    ...
    create_info.queueCreateInfoCount = (uint32_t)queues_create_infos.size();

    create_info.enabledExtensionCount = (uint32_t)m_required_extensions.size();
...
void Renderer::create_swap_chain() {
    ...
    if (details.capabilities.currentExtent.width == std::numeric_limits<uint32_t>::max()) {
        actual_extent.width = std::clamp((uint32_t)m_window.cam.window_width, details.capabilities.minImageExtent.width, details.capabilities.maxImageExtent.width);
        actual_extent.height = std::clamp((uint32_t)m_window.cam.window_height, details.capabilities.minImageExtent.height, details.capabilities.maxImageExtent.height);
    }
...
void Renderer::create_command_buffers() {
    ...
    cmd_alloc_info.commandBufferCount = (uint32_t)m_command_buffers.size();
...
void Renderer::create_descriptor_sets() {
    ...
    vkUpdateDescriptorSets(m_device, (uint32_t)all_descriptor_writes.size(), all_descriptor_writes.data(), 0, nullptr);
}
...
void Renderer::write_image(VkCommandBuffer command_buffer, uint32_t current_frame) {
    memcpy(m_image_buffers_mapped[current_frame], m_image_data, static_cast<size_t>(m_image_size));
